...
<?php 
    ...
    if(isset($_POST['submit'])) {
        ...
        if(empty($_POST['job-title']) OR empty($_POST['job-region']) OR empty($_POST['job-type'])) {
            ...
            $search = $conn->query("SELECT * FROM jobs WHERE job_title LIKE '%$job_title%' AND job_region='$job_region' AND job_type='$job_type' AND status = 0");
        ...
        <ul class="job-listings mb-5">
            <?php if(count($searchRes) > 0) : ?>
                ...
                <li class="job-listing d-block d-sm-flex pb-3 pb-sm-0 align-items-center">
                    ...
                    <img src="users/user-images/<?php echo $oneJob->company_image; ?>" alt="JOB BY COMPANY" class="img-fluid">
                    </div>
